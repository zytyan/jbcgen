...
```text
source bytes ──► documentation comments
Clang JSON AST ─► C declarations and types
               │
               ▼
...
生成的公开 wrapper 通过宏引用版本化符号 `json_reflect_check_abi_v1`，因此 ABI 主版本不匹配会在链接时失败；调用时再传入当前编译单元计算出的 64 位编译环境指纹。指纹覆盖基本类型宽度/对齐、reflection、parser、error、string 和 key-map 公开结构布局、对齐以及 plain `char` signedness，runtime 会拒绝不兼容调用。ABI 信息不存入每个类型描述符。共享库 SONAME 主版本为 `1`。生成目标必须与 runtime 和实际使用用户结构体的目标共享 packing、char signedness、target ABI 和控制结构体定义的宏；这些选项不一致属于不支持的构建。
