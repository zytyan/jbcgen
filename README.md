...
注解必须位于 UTF-8 源文件的文档注释中，支持 `///`、`//!`、`/** ... */` 和 `/*! ... */`。生成器按 Clang AST 中的真实声明位置读取入口头文件及其被包含头文件的原始字节，并在 Python 侧扫描、缓存和关联注释；Clang AST 仅用于解析 C 声明和类型。宏展开生成的注释不受支持。字段支持尾随 `/// @json(...)`。未知命令、未知参数、非法重复参数及不适用的组合会在生成期报告实际头文件、行和列。
